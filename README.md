...
- **Push notifications** — subscribe on any device, receive alerts even in the background; see [docs/notifications.md](docs/notifications.md)
...
> If `VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY` are **not** set, the server auto-generates them and saves them to `.vapid-keys.json` on first run. This works fine for a single container, but means push subscriptions are lost if the container is recreated. See [docs/notifications.md](docs/notifications.md) for full push setup.
...
See **[docs/notifications.md](docs/notifications.md)** for full setup, subscriber management, and troubleshooting.
...
See **[docs/notifications.md → Setup — Cloudflare Pages](docs/notifications.md#setup--cloudflare-pages-serverless)** for full VAPID key setup and environment variables.

---

## 🎵 Music Setup

See **[docs/music.md](docs/music.md)** for format support, server playlist setup, and local folder picker instructions.
...
|----------|-------------|
| [docs/notifications.md](docs/notifications.md) | Push notification setup, user options, daily weather summary, and troubleshooting |
| [docs/music.md](docs/music.md) | Music folder setup, supported formats, playlist generation |
...
> **Note:** VAPID keys are auto-generated on first run and persisted to `.vapid-keys.json` (gitignored). This is fine for a single container. To preserve push subscriptions across container recreations, set `VAPID_PUBLIC_KEY` and `VAPID_PRIVATE_KEY` as environment variables instead. See [docs/notifications.md](docs/notifications.md) for the full setup guide.
